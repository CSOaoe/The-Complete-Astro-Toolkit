...
import { bestTargetsTonight, moonPhase } from "@/utils/astronomy";
...
export default function TonightScreen() {
  ...
  const { favourites, toggleFavourite, observer, error } = useAppData();
  const reference = useMemo(() => new Date(), []);
  const recommendations = useMemo(
    () => bestTargetsTonight(targets, observer, reference),
    [observer, reference],
  );
  ...
  return (
    <Screen>
      ...
      <SectionHeader
        title="Best targets tonight"
        subtitle="Ranked by peak altitude between 18:00 and 06:00"
      />
      {recommendations.map(
        ({ target, currentAltitude, maximumAltitude, bestTime }) => (
          <Pressable
            key={target.id}
          ...
          >
            <Card>
              ...
              <View style={styles.stats}>
                <Stat
                  label="ALTITUDE NOW"
                  value={`${Math.round(currentAltitude)}°`}
                />
                <Stat
                  label="PEAK TONIGHT"
                  value={`${Math.round(maximumAltitude)}°`}
                />
                <Stat label="BEST TIME" value={bestTime} />
              </View>
      ...
      <Text style={styles.disclaimer}>
        Visibility is an astronomical estimate. Local terrain, buildings,
        weather and twilight are not included.
      </Text>
    </Screen>
  );
}

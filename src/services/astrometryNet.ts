...
async function json<T>(response: Response): Promise<T> {
  if (!response.ok) throw new Error(`Astrometry.net returned ${response.status}.`);
  const value = (await response.json()) as T & { status?: string; errormessage?: string };
  if (value.status === "error") throw new Error(value.errormessage || "Astrometry.net could not process the request.");
  return value;
}
...
export async function solveImage(
  ...
  onStatus?.("Signing in to Astrometry.net…");
  const loginBody = new URLSearchParams({ "request-json": JSON.stringify({ apikey: apiKey.trim() }) });
  ...
  if (jobId === null) throw new Error("The solve timed out before Astrometry.net assigned a job.");
  for (let attempt = 0; attempt < 60; attempt += 1) {
    onStatus?.("Solving and calibrating the frame…");
    const job = await json<{ status: string }>(await fetch(`${API}/jobs/${jobId}`));
    if (job.status === "failure") throw new Error("Astrometry.net could not recognise enough stars in this image.");
